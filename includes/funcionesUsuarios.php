...
function traerUsuarios() {
	$sql = "select u.idusuario,u.usuario, u.password, r.descripcion, u.email , u.nombrecompleto, u.cedula, (case when u.terminoscondiciones = 1 then 'Si' else 'No' end) as terminoscondiciones, u.refroles
			from dbusuarios u
...
function traerUsuarioId($id) {
	$sql = "select idusuario,usuario,refroll,nombrecompleto,email,password cedula, 
		(case when terminoscondiciones = 1 then 'Si' else 'No' end) as terminoscondiciones from dbusuarios where idusuario = ".$id;
	$res = $this->query($sql,0);
	if ($res == false) {
		return 'Error al traer datos';
	} else {
